...
export default {
  ...
  async onSubmit() {
    try {
      ...
      const response = await this.$store.dispatch('auth-module/loginAdmin', loginData)

	  if (response.user.role.name === 'admin' || response.user.role.name === 'super_admin') {
