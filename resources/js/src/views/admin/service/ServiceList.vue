    ...
    >
      ...
      >
        ...
        <template #cell(actions)="data">
          ...
          />
          <feather-icon
            :id="`service-row-${data.item.id}-supprimer-icon`"
            icon="TrashIcon"
            size="16"
            class="mr-1 cursor-pointer"
          ...
          <b-tooltip
            title="Supprimer"
            :target="`service-row-${data.item.id}-supprimer-icon`"
          />
        </template>
...
import ServiceAddEditSidebar from './ServiceAddEditSidebar.vue'
    ...
    deleteRecord(selectedRecord) {
      if (selectedRecord.id !== 0) {
        this.$bvModal.msgBoxConfirm('Êtes-vous sûr de vouloir supprimer ce service?', {
          ...
          .then(value => {
            if (value) {
              store.dispatch('service-module/deleteService', selectedRecord.id).then(() => {
                this.toast({
                  component: ToastificationContent,
                  props: {
                    title: 'Le service a été supprimée avec succès!',
