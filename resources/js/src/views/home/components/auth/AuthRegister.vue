<template>
  <div>
	...
	<b-modal id="success-modal" centered hide-footer>
		<div class="text-center p-3">
			<feather-icon icon="CheckCircleIcon" size="50" class="text-success mb-2"/>
			<h3>Inscription réussie 🎉</h3>
			<p>Votre compte a été créé avec succès</p>

			<b-button variant="success" @click="$bvModal.hide('success-modal')">
				Continuer
			</b-button>
		</div>
	</b-modal>
			...
			google.accounts.id.initialize({
				...
				callback: async (resp) => {
					...
					try {
						...
						// this.$toast.error(error.response.data.errors ? Object.values(error.response.data.errors).join('<br>') : 'Impossible de se connecter via Google')
					}
				}
			});
